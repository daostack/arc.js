...
export const createAddProposal = async (arc: Arc, dao: DAO, plugin: PluginManagerPlugin, options: IProposalCreateOptionsPM) => {
  ...
  dao.plugins().subscribe(plugins => {
    ...
    if(result) newPlugins.push(result as AnyPlugin)
  })
...
export const voteProposal = async (arc: Arc, proposal: PluginManagerProposal, dao: DAO) => {
